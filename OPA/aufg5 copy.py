...
plt.errorbar(g_nom,beta_nom,xerr=g_std,yerr=beta_std, fmt='o',label='Messwerte')
plt.plot(func_g(np.linspace(np.min(beta_nom), np.max(beta_nom)) ,*popt_g),np.linspace(np.min(beta_nom), np.max(beta_nom)) ,label=f"Fit g mit $f = {round_errtex(popt_g[0],errors_g[0])}$ and $h_1 = {round_errtex(popt_g[1],errors_g[1])}$")
# axis labels
plt.xlabel('g in cm')
plt.ylabel('beta')
plt.legend()
...
plt.errorbar(g_nom,beta_nom,xerr=g_std,yerr=beta_std, fmt='o',label='Messwerte')
plt.plot(func_g_prime(np.linspace(np.min(beta_nom), np.max(beta_nom)) ,*popt_g_prime),np.linspace(np.min(beta_nom), np.max(beta_nom)) ,label=f"Fit g' with $f' = {round_errtex(popt_g_prime[0],errors_g_prime[0])}$ and $h_2 = {round_errtex(popt_g_prime[1],errors_g_prime[1])}$")
# axis labels
plt.xlabel("g' in cm")
plt.ylabel('beta')
plt.legend()
